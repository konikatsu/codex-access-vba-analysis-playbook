...
- 起動直後の外部DB接続が失敗してDBを開けなくなる構成があるため、対象Access資産へ触る前に、依頼元へ自動起動の有無、既知の起動経路、既存の無効化方法、検証済みbaseline、既知の副作用を必ずヒアリングする。依頼文に記載済みでも復唱確認し、完了まではコピーや調査を始めない。
- 本体DBを直接触らず、必ず作業コピーで検証する。
- 必須ヒアリング後、作業担当は実装前に自動起動の有無、経路、無効化方法、検証証跡、採用baselineを宣言する。`未確認`のまま実装へ進まない。
