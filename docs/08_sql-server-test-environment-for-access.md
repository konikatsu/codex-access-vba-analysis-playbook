...
Docker方式では、接続先を `サーバ名,ポート番号` 形式にします。以下のポート番号は構築例であり、案件ごとに割り当てます。
...
4. プロジェクトごとに、コンテナ名・ポート番号・volume名を分け、採用値をINIとstage記録の両方へ残す。
...
AccessのINIが停止中の接続先を指し、AutoExecや起動フォームが直ちに接続する場合は、Access自体が入口で停止することがあります。通常起動を繰り返さず、[起動処理を止めて開発モードで開く](requirements/01_startup-bypass.md)に従います。
