...
3. DAOとStartupProbeでAutoExec、起動フォーム、起動プロパティ、呼出先関数を確認する。
4. 呼出先関数に完全一致の`SKIP_AUTOEXEC`分岐がなければ作業コピーへ一度だけ追加し、既にあれば追加しない。
5. 通常起動とスキップ起動を確認し、自動起動無効化対応版を開発baselineにする。
6. VBAモジュールを `SaveAsText` で退避する。
7. API宣言とポインタ/ハンドル変数を64bit向けに修正する。
8. `LoadFromText` で作業コピーへ戻す。
9. もう一度 `SaveAsText` して、未変換の `Declare` や型不一致候補がないか確認する。
10. Accessプロセスと `.laccdb` が残っていないことを確認する。
11. 進捗メモへ、対象ファイル、修正モジュール、確認結果、未確認事項を記録する。
