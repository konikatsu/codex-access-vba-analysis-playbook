...
対象Access資産へ触る前に、依頼元へ必ず確認します。質問と完了条件の正典は[Access作業共通ルール](00_access-work-common-rules.md#前提条件-依頼元へ自動起動情報を必ずヒアリングする)です。

以下は要約です。正確な文言は正典を参照します。
...
回答の`不明`は認めます。ヒアリング完了後はセクション1の安全な事前調査にだけ進み、そこで確定します。最終判定が`未確認`のまま実装へ進みません。ヒアリング直後に原本の作業前SHA-256を取得し、完了時に再取得します。
...
実装前に[共通ルールの作業前ゲート](00_access-work-common-rules.md#05-作業前ゲートを必ず書く)を満たし、依頼元へ次を報告します。
...
`AllowBypassKey=False`では仮想Shiftを使わず、[詳細リファレンスの例外時の切替](17_access-development-workflow-reference.md#12-例外時の切替)へ進みます。信頼済み場所はGUI/VBE・自己テスト専用stageに限定し、原本や静的Export・救出解析のコピーを置く親フォルダ全体を信頼しません。
