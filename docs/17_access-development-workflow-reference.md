...
否定試験の記録には、正規allowlistとは別に2通りの否定試験用allowlistのSHA-256を残します。`selftest_dispatcher_contract_manifest_sha256`は、AutoExec、起動関数、自己テストディスパッチャー、照合ヘルパーのSaveAsTextと、コード外に置く正規化規則・対応表の相対パスとSHA-256を列挙するmanifestのSHA-256です。正規化規則をmanifest外に置きません。manifestはUTF-8、BOMなし、LF改行のcompact JSON配列とし、相対パスを`/`区切りのordinal昇順に並べ、各要素を`relative_path`、`byte_length`、`sha256`の固定順で余分な空白なく書きます。`sha256`は小文字64桁の16進数とし、末尾改行を1つ付けます。このバイト列のSHA-256を記録します。
...
取得した通信は、試験前に作成してSHA-256を固定した`selftest_observer_config`で分類します。canaryを使う経路では否定試験時間窓の受付件数が0であることを確認します。通信トレースを使う経路では、棚卸し済みの業務接続先または分類不能な宛先への試行が1件もないことを確認します。事前列挙したOS・Office基盤通信の分類は外向き通信の許可を意味しません。隔離stageでは外向き通信を遮断したまま試行を記録し、成功した外向き通信があれば`FAIL`とします。顧客接続先を基盤通信として登録しません。canaryはTCPの接続受付をログイン処理より前から数えますが、サーバー側のログイン監査はログイン処理へ到達しなかった試行を取りこぼし得るため、単独では証拠にしません。

ファイル接続では、否定試験のためだけに保存済みリンクを書き換えず、AccessのPIDと実行時間窓だけで絞ったOS側ファイルアクセス記録から全open試行を取得します。作業ACCDB、対応INI、`.laccdb`または`.ldb`、結果JSON、allowlist、および`selftest_observer_config`へ事前列挙したローカル一時・システム依存先以外のパスが0件であることを確認し、取得前にパスフィルターをかけません。実行時に検証先を差し替えられる経路では、検証専用のcanaryパスも使用できます。
...
`selftest_negative_test_evidence_sha256`は、制御試験、2通りの否定試験、通信・ファイル観測の各ログと結果JSONを列挙した証跡manifestのSHA-256です。証跡manifestは前述のcontract manifestと同じcanonical形式で作ります。`selftest_negative_test_status`を`PASS`にできるのは、必要な制御試験と2通りの否定試験がすべて合格し、manifestと各証跡のSHA-256が一致する場合だけです。それ以外は`FAIL`、`要実機確認`、または`未検証`とします。
