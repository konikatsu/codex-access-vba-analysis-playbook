...
本書の「外部接続」と「業務データ接続」は、AccessアプリケーションがINI、`Connect`、SQL、VBAなどから開くデータベース、ファイル、APIその他の業務依存先への接続を指します。「基盤通信」はOSまたはOfficeがアプリケーションの業務処理とは独立して行う通信を指し、外部接続棚卸しには含めません。ただし、自己テストの観測では取得、分類、遮断確認の対象から除外しません。
...
結果には自然終了の待機時間、強制停止の有無、identity不一致、PIDが空いたか、lock待機時間を残します。直後の外側確認でPIDが消えていても、即時判定で`FAIL`となった実行を後から`PASS`へ昇格させず、新しいコピーと修正済みハーネスで一度だけやり直します。
...
selftest_dispatcher_contract_manifest_sha256
...
- 読み取り系・更新系とも、最初の業務データ接続を開く前に、アプリ自身が接続設定から導出した接続先をアプリケーションINIとは独立したallowlistと完全一致で照合する。
...
DB側ディスパッチャーは、環境変数やJSONが空、不正、重複、不明な種類、想定外の`schema_version`、必須項目不足、ワイルドカードを含む場合に接続前に`FAIL`とします。INI、`Connect`、保存SQLなどから接続先を接続せずに導出し、案件で定義した正規化後の`kind`と接続先項目を完全一致で比較します。正規化では接続種別ごとの識別項目を省略しません。サーバーDBではプロトコル、ホスト、インスタンス名またはポート、データベース名を、API等ではscheme、ホスト、実効ポート、接続先を識別するパスを脱落させません。大文字小文字などを同一視するのは、使用するドライバーと接続先が同一と定義する項目だけに限定します。`host\instance`と`host,port`、DNS名と解決後IP、UNCとドライブ文字は、同一性を別途証明して明示的な対応表に固定しない限り同一視しません。ファイルは絶対パスへ変換し、相対パスを受け付けません。部分一致やDSN名だけの一致を認めません。不一致なら結果JSONへ`target_allowlist_match=false`を記録してAccessを閉じ、業務データ接続を開きません。

`target_allowlist_match=true`はDB側の自己申告なので、それだけを接続前照合の証明にしません。ディスパッチャーのコードレビューに加え、検証専用コピーで、空または不正JSONのallowlistと、形式は正しいが接続先が一致しないallowlistの2通りを否定試験します。どちらも業務データ接続を開く前に`FAIL`となることを確認します。

否定試験の記録には、正規allowlistとは別に2通りの否定試験用allowlistのSHA-256を残します。`selftest_dispatcher_contract_manifest_sha256`は、AutoExec、起動関数、自己テストディスパッチャー、照合ヘルパーのSaveAsTextと、コード外に置く正規化規則・対応表の相対パスとSHA-256を列挙するmanifestのSHA-256です。正規化規則をmanifest外に置きません。

contract manifestは`selftest_contract`ディレクトリへ置き、各manifest自身の格納ディレクトリを相対パスの基準とします。manifestファイル自身は列挙対象に含めません。UTF-8、BOMなし、LF改行のcompact JSON配列とし、相対パスはUnicode正規化せずファイルシステムから得た文字列を使い、`.`と`..`を含めず、区切りを`/`へ統一してUTF-8バイト列の昇順に並べます。各要素は`relative_path`、`byte_length`、`sha256`の固定順で、要素間を含め余分な空白なく書きます。文字列では`"`を`\"`、`\`を`\\`、U+0000からU+001Fを小文字16進の`\u00xx`でエスケープし、それ以外の非ASCII、`/`、`<`、`>`、`&`、`'`はエスケープせずUTF-8で書きます。`sha256`は小文字64桁の16進数とし、JSON配列の直後にLFを1つだけ付けます。このバイト列のSHA-256を記録します。

前回の否定試験を再利用できるのは、そのmanifest、ラッパー、allowlistのschemaと正規化規則、対象操作の接続生成経路、Accessのバージョンとbitness、使用ドライバー、および観測方法がすべて一致し、2通りの入力と証跡のSHA-256を再確認できる場合だけです。どれかが変わるか証明できなければ、否定試験をやり直します。

業務データ接続の不在は結果JSONだけでは証明できません。INIなどから実行時に接続文字列を組み立て、`RefreshLink`なしで検証先を差し替えられる経路では、制御下のローカルcanaryへ向けて接続受付件数が0であることを記録できます。保存済みODBCリンクテーブルやパススルークエリの経路では、否定試験のためだけに`Connect`を書き換えたり`RefreshLink`したりせず、Access起動前からプロセス生成を記録します。AccessのPID、その時間窓内にAccessから生成されたことを記録で証明できる子孫プロセスのPID、および実行時間窓で絞ったホスト側通信トレースにより、外向き接続試行をすべて取得します。取得前に宛先フィルターをかけません。

取得した通信は、試験前に作成してSHA-256を固定した`selftest_observer_config`で分類します。試験開始前に、OS・Office基盤通信として列挙した宛先と名前解決の照会名を、接続棚卸し、正規・否定試験用allowlist、および候補から導出した接続先と機械照合します。1件でも重なるか照合不能なら試験を開始しません。照合結果を証跡manifestへ含め、顧客接続先を基盤通信として登録しません。

外向き通信ポリシーは時間窓ごとに分けます。否定試験時間窓ではループバックとローカルcanaryだけを遮断対象外とし、それ以外の外向き通信を遮断します。観測手段の制御試験は別の時間窓で同じ遮断を維持し、制御用プロセスから、(1) ループバックcanaryへの接続、(2) 実在する業務・基盤・検証先と重ならない制御用の非ループバック宛先への接続試行、(3) 実在する業務・基盤名と重ならない制御用ホスト名の名前解決照会を発生させます。(2)と(3)は制御用プロセスが生成した子プロセスからも発生させます。制御用プロセスと子孫プロセスのPID、および制御試験時間窓を、Access側と同じ条件で記録して絞り込み、(1)を受付、(2)を遮断された試行、(3)を照会名として各観測経路から検出できることを確認します。制御用の宛先と照会名は`selftest_observer_config`に固定し、試験前の機械照合に含めます。正の自己テストはさらに別の時間窓で、正規allowlistと完全一致した検証用接続先だけを通信許可し、それ以外を遮断します。全遮断を無条件に解除しません。各モードの遮断範囲、開始・終了時刻、例外、切替結果は`selftest_observer_config`と観測ログへ記録します。

canaryを使う経路では否定試験時間窓の受付件数が0であることを確認します。通信トレースを使う経路では、棚卸し済みの業務接続先または分類不能な宛先への試行が1件もないことを確認します。名前解決はリゾルバーの宛先だけで分類せず、Accessまたは記録済み子孫プロセスのPIDと実行時間窓に関連付けた照会名を宛先フィルター適用前に取得します。棚卸し、正規・否定試験用allowlist、または候補から導出した業務ホスト名に一致する照会、および分類不能な照会が1件でもあれば、業務接続先への試行として`FAIL`にします。照会名を取得して制御試験で検出能力を証明できない観測方法では、ホスト名を使う経路の否定試験を合格にしません。`FollowHyperlink`、DDE、COM、シェル起動などが既存プロセスへ処理を委譲し、その通信を当該実行へ帰属できない場合も、PIDによる観測だけで否定試験を合格にしません。事前列挙したOS・Office基盤通信の分類は通信許可を意味せず、否定試験中は遮断したまま試行を記録します。業務接続先または分類不能な宛先への試行が1件でもあれば否定試験を`FAIL`とし、基盤通信を含む非ループバック通信が1件でも成功していれば遮断自体も`FAIL`とします。canaryはTCPの接続受付をログイン処理より前から数えますが、サーバー側のログイン監査はログイン処理へ到達しなかった試行を取りこぼし得るため、単独では証拠にしません。

ファイル接続では、否定試験のためだけに保存済みリンクを書き換えず、Accessと記録済み子孫プロセスのPID、および実行時間窓で絞ったOS側ファイルアクセス記録から全open試行を取得します。作業ACCDB、対応INI、`.laccdb`または`.ldb`、結果JSON、allowlist、および`selftest_observer_config`へ事前列挙したローカル一時・システム依存先以外のパスが0件であることを確認し、取得前にパスフィルターをかけません。事前列挙した依存先は、接続棚卸し、allowlist、および候補から導出したファイル接続先と試験前に機械照合し、重なるか照合不能なら試験を開始しません。実行時に検証先を差し替えられる経路では、検証専用のcanaryパスも使用できます。

ネットワークとファイルの各観測手段は、否定試験とは別の時間窓で制御用アクセスを発生させ、検出できることを先に確認します。タイムアウトまでの長さだけ、結果JSONが`FAIL`であることだけでも、業務データ接続不在の証拠にしません。実値を入れたallowlist、`selftest_observer_config`、観測証跡は隔離stage内だけに保存し、公開リポジトリ、AIへの依頼、申し送りへ転記しません。
...
個々の否定試験が合格するのは、ラッパーが非PASSとして終了し、結果JSONが`status=FAIL`、`target_allowlist_match=false`、空でない`error_code`を持ち、起動したAccess PIDとロックが消え、canary受付、業務接続先・分類不能な宛先への通信試行、業務接続先・分類不能な名前解決照会、成功した非ループバック通信、許可外ファイルopenがすべて0件の場合だけです。非PASS時は参照ラッパーがPIDとロックの検査へ到達しないため、その消失は観測側または外側の実行ハーネスが確認し、証跡に含めます。いずれかが1件でもあるか証明できなければ、その否定試験は不合格です。`selftest_negative_test_status`を`PASS`にできるのは、必要な制御試験と2通りの否定試験がすべて合格し、manifestと各証跡のSHA-256が一致する場合だけです。それ以外は`FAIL`、`要実機確認`、または`未検証`とします。
...
- 自己テストラッパーとDB側ディスパッチャーcontract manifestのSHA-256、allowlist schema、正規・否定試験用allowlistのSHA-256、Access・ドライバー・観測設定の識別子、2通りの否定試験結果、業務データ接続不在の観測方法と証跡のSHA-256
- 自己テスト、GUI、帳票、PDFの証跡
- 既知の未検証項目
