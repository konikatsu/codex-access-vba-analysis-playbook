...
業務データ接続の不在は結果JSONだけでは証明できません。INIなどから実行時に接続文字列を組み立て、`RefreshLink`なしで検証先を差し替えられる経路では、制御下のローカルcanaryへ向けて接続受付件数が0であることを記録できます。保存済みODBCリンクテーブルやパススルークエリの経路では、否定試験のためだけに`Connect`を書き換えたり`RefreshLink`したりせず、AccessのPIDと実行時間窓だけで絞ったホスト側通信トレースにより、その時間窓の外向き接続試行をすべて取得します。取得前に宛先フィルターをかけません。

取得した通信は、試験前に作成してSHA-256を固定した`selftest_observer_config`で分類します。試験開始前に、OS・Office基盤通信として列挙した宛先を、接続棚卸し、正規・否定試験用allowlist、および候補から導出した接続先と機械照合します。1件でも重なるか照合不能なら試験を開始しません。照合結果を証跡manifestへ含め、顧客接続先を基盤通信として登録しません。

外向き通信ポリシーは時間窓ごとに分けます。否定試験時間窓ではループバックとローカルcanaryだけを遮断対象外とし、それ以外の外向き通信を遮断します。観測手段の制御試験は別の時間窓でループバックcanaryを使い、同じ遮断を維持したまま行います。正の自己テストはさらに別の時間窓で、正規allowlistと完全一致した検証用接続先だけを通信許可し、それ以外を遮断します。全遮断を無条件に解除しません。各モードの遮断範囲、開始・終了時刻、例外、切替結果は`selftest_observer_config`と観測ログへ記録します。

canaryを使う経路では否定試験時間窓の受付件数が0であることを確認します。通信トレースを使う経路では、棚卸し済みの業務接続先または分類不能な宛先への試行が1件もないことを確認します。事前列挙したOS・Office基盤通信の分類は通信許可を意味せず、否定試験中は遮断したまま試行を記録します。業務接続先または分類不能な宛先への試行が1件でもあれば否定試験を`FAIL`とし、基盤通信を含む非ループバック通信が1件でも成功していれば遮断自体も`FAIL`とします。canaryはTCPの接続受付をログイン処理より前から数えますが、サーバー側のログイン監査はログイン処理へ到達しなかった試行を取りこぼし得るため、単独では証拠にしません。

ファイル接続では、否定試験のためだけに保存済みリンクを書き換えず、AccessのPIDと実行時間窓だけで絞ったOS側ファイルアクセス記録から全open試行を取得します。作業ACCDB、対応INI、`.laccdb`または`.ldb`、結果JSON、allowlist、および`selftest_observer_config`へ事前列挙したローカル一時・システム依存先以外のパスが0件であることを確認し、取得前にパスフィルターをかけません。事前列挙した依存先は、接続棚卸し、allowlist、および候補から導出したファイル接続先と試験前に機械照合し、重なるか照合不能なら試験を開始しません。実行時に検証先を差し替えられる経路では、検証専用のcanaryパスも使用できます。
...
`selftest_negative_test_evidence_sha256`は、観測設定の事前照合、ポリシー切替、制御試験、2通りの否定試験、通信・ファイル観測の各ログと結果JSONを列挙した証跡manifestのSHA-256です。証跡manifestは前述のcontract manifestと同じcanonical形式で作ります。

個々の否定試験が合格するのは、ラッパーが非PASSとして終了し、結果JSONが`status=FAIL`、`target_allowlist_match=false`、空でない`error_code`を持ち、起動したAccess PIDとロックが消え、canary受付、業務接続先・分類不能な宛先への通信試行、成功した非ループバック通信、許可外ファイルopenがすべて0件の場合だけです。いずれかが1件でもあるか証明できなければ、その否定試験は不合格です。`selftest_negative_test_status`を`PASS`にできるのは、必要な制御試験と2通りの否定試験がすべて合格し、manifestと各証跡のSHA-256が一致する場合だけです。それ以外は`FAIL`、`要実機確認`、または`未検証`とします。
