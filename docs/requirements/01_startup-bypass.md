...
`AllowBypassKey=False`などで自動起動を確実に抑止できず、通常起動を検討する場合は、先に[通常起動前の外部接続棚卸し](../17_access-development-workflow-reference.md#231-通常起動前の外部接続棚卸し)を行います。起動経路から到達し得る接続元または接続先が不明なら、通常起動しません。
