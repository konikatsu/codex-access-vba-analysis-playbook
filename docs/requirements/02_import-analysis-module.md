...
この方法は解析専用コピーでの初見解析向けです。解析モジュールを取り込むとDBのSHA-256と資産構成が変わるため、正式なbaselineや候補には使いません。正式差分では[Access外部Exportツール](../16_access-external-export.md)を使用します。
