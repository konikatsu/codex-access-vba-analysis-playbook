...
Private Sub ExportTableDefinitions(ByVal filePath As String, ByVal logNo As Integer)
    ...
    For Each tdf In db.TableDefs
        If ShouldExportObject(CStr(tdf.Name)) Then
            ...
            ' Metadata only: do not use RecordCount, recordsets, domain aggregates, or RefreshLink.
            If Len(tdf.Connect & "") > 0 Then Print #fileNo, "  Connect: " & DescribeLinkConnection(CStr(tdf.Connect))
...
Private Function DescribeLinkConnection(ByVal connectString As String) As String
    Dim normalized As String

    normalized = UCase$(Trim$(connectString))

    If Left$(normalized, 5) = "ODBC;" Then
        DescribeLinkConnection = "ODBC (details redacted)"
    Else
        DescribeLinkConnection = "linked table (details redacted)"
    End If
End Function
